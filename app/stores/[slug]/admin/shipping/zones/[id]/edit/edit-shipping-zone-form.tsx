...
import { updateShippingZoneAction } from "../../../actions";
...
interface EditShippingZoneFormProps {
  zone: SelectShippingZone;
  storeId: string;
  slug: string;
}

export function EditShippingZoneForm({ zone, storeId, slug }: EditShippingZoneFormProps) {
  ...
  const { execute } = useAction(updateShippingZoneAction, {
    onSuccess: () => {
      toast.success("Shipping zone updated successfully");
      router.push(`/stores/${slug}/admin/shipping/zones/${zone.id}`);
    },
    onError: (error) => {
      toast.error(error.serverError || "Failed to update shipping zone");
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="flex gap-2 justify-end">
        <Button
          type="button"
          variant="outline"
          onClick={() => router.push(`/stores/${slug}/admin/shipping/zones/${zone.id}`)}
