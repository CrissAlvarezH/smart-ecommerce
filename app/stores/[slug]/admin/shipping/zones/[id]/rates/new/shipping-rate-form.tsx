...
import { createShippingRateAction } from "../../../../actions";
...
export function ShippingRateForm({ zoneId, slug }: ShippingRateFormProps) {
  ...
  const { execute } = useAction(createShippingRateAction, {
    onSuccess: () => {
      toast.success("Shipping rate created successfully");
      router.push(`/stores/${slug}/admin/shipping/zones/${zoneId}`);
    },
    onError: (error) => {
      toast.error(error.serverError || "Failed to create shipping rate");
