...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { success: false, message: "Seeding failed", error: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    console.log("🗑️ Clearing ecommerce data...");
    ...
    console.log("✅ All ecommerce data cleared!");

    return NextResponse.json({
      success: true,
      message: "All ecommerce data cleared successfully!"
    });

  } catch (error) {
    console.error("❌ Clearing failed:", error);
    return NextResponse.json(
      { success: false, message: "Clearing failed", error: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 }
    );
  }
}